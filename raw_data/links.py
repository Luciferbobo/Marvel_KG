...
p2=[]
for i in range(len(relation_pro)):
    p2.append({'source': num[relation_pro.iloc[i][0]]-1, 'target': num[relation_pro.iloc[i][1]], 'value': relation_pro.iloc[i][2]})
...
with open('relation.txt','a',encoding='utf-8') as f:
    for i in range(len(relation_pro)):
        text = relation_pro.iloc[i][0]+','+relation_pro.iloc[i][1]+','+relation_pro.iloc[i][2]+',贾家宁国府,贾家宁国府'+'\n'
        f.write(text)
